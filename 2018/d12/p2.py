...
def run_day():
    ...
    for x in range(1, loops + 1):
        ...
        total_offset += offset
        new_sum = count_pots(state, total_offset)
        change.append(new_sum - last_sum)
        last_sum = new_sum

    if GENERATIONS > 1000:
        change_avg = sum(change[-10:]) // 10
        return (GENERATIONS - 1000) * change_avg + count_pots(state, total_offset)
    else:
